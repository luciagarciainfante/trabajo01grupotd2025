...
author: "Grupo 3. Lucía García Infante, Rosa Fernández López, Sergio Pacheco Márquez, Víctor Silva Nieves"
format: 
  html:
    toc: true
    toc-depth: 3
...
```{r, echo= FALSE}
#Creamos la matriz
matriz_enunciado <- matrix(c(
  100,200,130,150,240,100,
...
tabla_enunciado <- as.data.frame(matriz_enunciado)

#Asignamos nombres de columnas (estados de la naturaleza) y filas (alternativas)
colnames(tabla_enunciado) <- c("e1", "e2", "e3", "e4", "e5", "e6")
rownames(tabla_enunciado) <- c("d1", "d2", "d3", "d4","d5")

#Mostramos la tabla con estilo
crea_tabla_estilo(tabla_enunciado, TRUE)
...
#Asignamos nombres de columnas (estados de la naturaleza) y filas (alternativas)
colnames(tabla_enunciado) <- c("e1", "e2", "e3")
rownames(tabla_enunciado) <- c("d1", "d2", "d3")
...
crea_tabla_estilo(sol_T_d, TRUE)
```
...
- Alternativas:
  - A1 = ElectroPlus (tarifa fija mensual de 45€).
  - A2 = LuzFlex (tarifa variable de 0,20€/kWh + cuota fija de 15€).
  - A3 = EcoVolt (tarifa variable de 0,15€/kWh + penalización de 10€ si se supera el umbral de 300kWh).
- Estados de la naturaleza:
  - E1 = Consumo moderado (250kWh/mes).
  - E2 = Consumo elevado (350kWh/mes).
- Matriz de decisión:  Costes anuales calculados según cada alternativa y escenario de consumo.
...
crea_tabla_estilo(sol_carlos, TRUE)
